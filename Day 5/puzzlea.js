...
for (let i = 0; i < puzzleArray.length; i++) {
    ...
    if (i == puzzleArray.length - 1) {
        for (let i = 0; i < positions.length; i++) {
            for (let j = 0; j < parseInt(positions[i][1]); j++) {
                let temp,
                    temp2 = [];
                temp = tool[parseInt(positions[i][3]) - 1];
                temp2 = tool[parseInt(positions[i][5]) - 1];
                let value = temp[0];
                temp.shift();
                temp2.unshift(value);
                tool[parseInt(positions[i][3]) - 1] = temp;
                tool[parseInt(positions[i][5]) - 1] = temp2;
            }
        }
